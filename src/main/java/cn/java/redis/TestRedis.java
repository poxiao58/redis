...
public class TestRedis {
	...
	@Before 
	public void setUp() { 
	pool = new JedisPool(new JedisPoolConfig(), "192.168.30.65");  //"192.168.30.65"为电脑的ip地址
	...
	*/ 
	...
	//-----添加数据---------- 
	jedis.set("name","lijun");//向{key:value}-->{name:lijun} 
	System.out.println(jedis.get("name"));//执行结果：lijun 
	//-----修改数据----------- 
	//1、在原来基础上修改 
	jedis.append("name","useredis");   //将useerdis append到已经有的value之后 
	System.out.println(jedis.get("name"));//执行结果:lijunuseredis  
	//2、直接覆盖原来的数据 
	jedis.set("name","李俊"); 
	System.out.println(jedis.get("name"));//执行结果：李俊
	//删除key对应的记录 
	jedis.del("name"); 
	...
	* jedis.set("lijun","李俊"); 
	*/ 
	jedis.mset("name","minxr","lijun","李俊");            
	System.out.println(jedis.mget("name","lijun"));  
	} 
	/** 
	* jedis操作Map 
	*/ 
